...
| Asset manifest and CI contract omitted recovered source changes | Medium | Hosted Python suite identified stale registry hash and old build-command expectation. Canonical builder regenerated only timestamp and registry hash, preserving all 143 asset bindings; CI contract now explicitly requires asset and critical-CSS build/check commands. Focused manifest/CI tests pass (23 passed, seven unrelated Prettier checks skipped because root node_modules is absent); actual theme asset and critical-CSS checks pass. |

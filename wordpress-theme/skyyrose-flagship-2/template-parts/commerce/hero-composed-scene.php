...
$srcset            = array();
// Backdrop is a CSS hook only: the approved collection hero sits blurred behind plates whose ratio
// differs from the viewport, so portrait plates read as one scene without touching hotspot geometry.
$backdrop_collection = skyyrose2_collections()[ $collection ] ?? array();
$backdrop_uri        = ! empty( $backdrop_collection['hero'] ) ? skyyrose2_sot_asset_uri( $backdrop_collection['hero'] ) : '';
...
?>
<article class="sr2-world sr2-world--commerce sr2-world--hero-composed" style="--scene-ratio: <?php echo esc_attr( (string) ( absint( $scene['width'] ) / max( 1, absint( $scene['height'] ) ) ) ); ?>;
<?php
if ( $backdrop_uri ) :
	?>
	--sr2-scene-backdrop: url(<?php echo esc_url( $backdrop_uri ); ?>);<?php endif; ?>" data-scene-id="<?php echo esc_attr( $scene_id ); ?>" data-product-state="<?php echo esc_attr( $products['state'] ); ?>" data-composite-state="<?php echo empty( $scene['scene_motion'] ) ? 'candidate' : 'founder-approved-local-motion'; ?>" data-product-count="<?php echo esc_attr( count( $scene['product_bindings'] ) ); ?>">
